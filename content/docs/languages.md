...
arbitrary API choice: `Node` is a fixed 152-byte struct with a size ratchet enforced by a test
...
already carries once, on `Tree.Language()`. (152 bytes/node is also the exact figure behind the
"honest asterisk" on the [Performance](/docs/performance) page: it's larger than C tree-sitter's
16–64 bytes/node, and this is why.)
